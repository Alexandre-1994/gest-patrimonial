...
@section('content')
    <div class="container-fluid px-4">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    ...
                    <h2 class="page-title">Editar Movimentação de Ativo</h2>
                </div>
                <div class="col-auto">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('asset-movements.update', $assetMovement->id) }}" method="POST"
                                    ...
                                    <select class="form-select @error('asset_id') is-invalid @enderror" id="asset_id"
                                        ...
                                        @foreach ($assets as $asset)
                                            <option value="{{ $asset->id }}"
                                                {{ $assetMovement->asset_id == $asset->id ? 'selected' : '' }}>
                                                {{ $asset->code }} - {{ $asset->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <label for="asset_id">Selecione o Ativo</label>
                                    @error('asset_id')
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <input type="text" class="form-control @error('from_location') is-invalid @enderror"
                                        ...
                                        value="{{ old('from_location', $assetMovement->from_location) }}" required>
                                    <label for="from_location">Localização de Origem</label>
                                    @error('from_location')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                    ...
                                    <select class="form-select @error('from_cost_center_id') is-invalid @enderror"
                                        ...
                                        @foreach ($costCenters as $costCenter)
                                            <option value="{{ $costCenter->id }}"
                                                {{ $assetMovement->from_cost_center_id == $costCenter->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <input type="text" class="form-control @error('to_location') is-invalid @enderror"
                                        ...
                                        value="{{ old('to_location', $assetMovement->to_location) }}" required>
                                    <label for="to_location">Localização de Destino</label>
                                    @error('to_location')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <select class="form-select @error('to_cost_center_id') is-invalid @enderror"
                                        ...
                                        @foreach ($costCenters as $costCenter)
                                            <option value="{{ $costCenter->id }}"
                                                {{ $assetMovement->to_cost_center_id == $costCenter->id ? 'selected' : '' }}>
                                                {{ $costCenter->code }} - {{ $costCenter->name }}
                                            </option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="form-section mb-4">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <input type="date" class="form-control @error('movement_date') is-invalid @enderror"
                                        ...
                                        value="{{ old('movement_date', $assetMovement->movement_date ? \Carbon\Carbon::parse($assetMovement->movement_date)->format('Y-m-d') : '') }}"
                                        required>
                                ...
                                <div class="form-floating">
                                    <select class="form-select @error('status') is-invalid @enderror" id="status"
                                        ...
                                        <option value="pending"
                                            {{ $assetMovement->status == 'pending' ? 'selected' : '' }}>Pendente</option>
                                        <option value="in_transit"
                                            {{ $assetMovement->status == 'in_transit' ? 'selected' : '' }}>Em Trânsito
                                        </option>
                                        <option value="completed"
                                            {{ $assetMovement->status == 'completed' ? 'selected' : '' }}>Concluído
                                        </option>
                                        <option value="cancelled"
                                            {{ $assetMovement->status == 'cancelled' ? 'selected' : '' }}>Cancelado
                                        </option>
                                    </select>
                            ...
                            <div class="col-12">
                                <div class="form-floating">
                                    <textarea class="form-control @error('reason') is-invalid @enderror" id="reason" name="reason" style="height: 100px"
                                        required>{{ old('reason', $assetMovement->reason) }}</textarea>
                            ...
                            <div class="col-12">
                                <div class="form-floating">
                                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes"
                                        style="height: 100px">{{ old('notes', $assetMovement->notes) }}</textarea>
                    ...
                    <div class="form-footer">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-2"></i>Atualizar Movimentação
                        </button>
